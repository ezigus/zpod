...
- Observed GitHub Actions failure in `UITests-SwipeConfiguration` after decomposition; linker reported missing `zpod.app/zpod` while embedding `IntegrationTests.xctest` during UI test builds.
- Root cause: `zpod.xctestplan` still referenced the `IntegrationTests` bundle, forcing the UI scheme to embed the integration test host when running UI suites.
- Fix: removed `IntegrationTests` entry from `zpod.xctestplan`; integration coverage remains via the dedicated `IntegrationTests` scheme job.
...
- Pushed fix to `copilot/decompose-core-workflow-tests`; rerun CI expected to pass the swipe configuration matrix.
