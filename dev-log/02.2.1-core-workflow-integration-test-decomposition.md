...
## 2025-10-28 07:45 ET – CI swipe UI failure triage
- Observed GitHub Actions failure in `UITests-SwipeConfiguration`; linker reported missing `zpod.app/zpod` while embedding `IntegrationTests.xctest` during UI test builds.
- Root cause: a previous change had removed `IntegrationTests` from `zpod.xctestplan`, so UI-only invocations no longer built the host app before embedding the integration bundle.
- Fix: restored the `IntegrationTests` entry in `zpod.xctestplan` so the build dependency graph stays intact while the dedicated integration CI job continues to run via its own scheme.
- Verified locally with `./scripts/run-xcode-tests.sh -t zpodUITests/SwipeConfigurationPersistenceUITests,zpodUITests/SwipeConfigurationExecutionUITests,zpodUITests/SwipeConfigurationPresetCyclingUITests,zpodUITests/SwipeConfigurationActionManagementUITests` ✔️
- Pushed update on `copilot/decompose-core-workflow-tests`; swipe matrix CI run now passes without linker errors.

## 2025-10-28 19:25 ET – Swipe preset teardown adjustment
- Follow-up CI run showed `SwipeConfigurationPresetCyclingUITests` occasionally failing while attempting to terminate the host app at tear-down.
- Relaxed `tearDownWithError` in `SwipeConfigurationUITests` to simply nil out `app`; the harness already force-terminates before each launch via `launchConfiguredApp`, so explicit termination is redundant.
- Local swipe suite run confirmed no regressions and the failure condition disappeared.
