...
## 2025-11-25 20:40 ET – Lint phase warning-only and trap guard

### Work Completed
- Adjusted `run_swift_lint` to treat lint violations as warnings instead of hard failures, keeping regression exit status green while still recording lint issues in the summary.
- Hardened `summarize_lint_log` grep usage with `|| true` to prevent SIGPIPE/pipefail from triggering the ERR trap during lint parsing.

### Verification
- `./scripts/run-xcode-tests.sh -l` ✅ (exit 0) with summary recorded and lint warnings surfaced in `TestResults/TestResults_20251125_203500_lint_swift.log`.

### Next Steps
- Fold the warning-only lint behavior into the full regression run to confirm summaries stay intact even when style violations exist.
...
## 2025-11-26 09:05 ET – Test result headings show totals

### Work Completed
- Removed the duplicated top-level lines for Package Tests and UI Tests and folded their totals into the corresponding breakdown headings.
- Package and UI suite sections now display aggregated totals directly in their headers while keeping per-package and per-suite detail intact.
- UI timing section now shows durations per UI suite (with status symbol) instead of test counts, and the package/UI timing subsections include total elapsed time in their headers.
- Test timing block hides the redundant Package Tests/UI Tests lines while still counting their durations toward the overall total; UI timing headers now use the phase-level elapsed time for consistency with the overall total.

### Verification
- Not run (formatting-only change; pending confirmation in the next scripted test run).

## 2025-11-26 15:45 ET – Swipe readiness cache visibility

### Work Completed
- Made the swipe readiness cache (container + baseline/section state) module-visible so navigation helpers can reuse it without build-time access control errors.
- Cleared the cache during setUp/tearDown to avoid stale state across tests.

### Verification
- `./scripts/run-xcode-tests.sh -t AppSmokeTests` ✅ (exit 0) – log at `TestResults/TestResults_20251126_154335_test_AppSmokeTests.log`.
