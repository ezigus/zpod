...
- Wired a single retry path that resets CoreSimulator and retries on a fresh simulator (retry capped at one attempt across failure types).
...
- `./scripts/run-xcode-tests.sh --self-check` after retry-cap adjustments.
