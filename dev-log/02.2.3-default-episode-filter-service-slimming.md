...
- Unit-test job now restores the shared host artifact and runs AppSmoke with `ZPOD_TEST_WITHOUT_BUILDING=1`, preventing another local rebuild on CI.

## 2025-10-31 09:25 ET – Test Coverage Gap Follow-up
- Open review identified missing unit tests for the newly extracted helpers (`EpisodeSearchHelper`, `AdvancedSearchEvaluator`, `SmartListRuleEvaluator`).
- Plan:
  - Add focused specs in `CoreModelsTests` mirroring Given/When/Then from episode filtering and smart list scenarios.
  - Cover positive/negative search matches, highlight generation, boolean operator handling, and smart list rule evaluations (dates, numbers, booleans).
  - Re-run `./scripts/run-xcode-tests.sh -t CoreModelsTests` to validate the new coverage.

```mermaid
sequenceDiagram
    participant Dev as Developer
    participant Tests as CoreModelsTests
    participant Helpers as Helper Structs
    Dev->>Helpers: Define example episodes/smart rules
    Dev->>Tests: Implement Given/When/Then cases
    Tests->>Helpers: Invoke search & rule evaluators
    Helpers-->>Tests: Return matches/highlights/results
    Tests-->>Dev: Assert expectations & document gaps
```

## 2025-10-31 14:45 ET – SwiftLint Compliance Follow-up
- Lint pass flagged the expanded swipe action APIs for exceeding the function parameter cap.
- Introduced `SwipeActionCallbacks` container so `SwipeActionHandler.performSwipeAction` now accepts three parameters while preserving existing behaviour.
- Updated `EpisodeListViewModel` wiring and `SwipeActionHandlerTests` to use the new callback set with zero boilerplate via default no-op closures.
- Re-running the full `./scripts/run-xcode-tests.sh` after the refactor to confirm lint + regression succeed.
