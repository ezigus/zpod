...
## 2025-10-21 10:15 ET - CI Parallelization Rethink (Design Intent)

**Context:**
- Serialized UI/Integration legs in CI were introduced to avoid simulator contention, but they now stretch wall-clock time to hours and hangs still reproduce (PlaybackUI, BatchOperations, Integration).
- Need to re-enable parallel execution without reintroducing collisions on the same CoreSimulator device.

**Research Highlights:**
- Reviewed Apple’s "Build reliable UI tests" session and GitHub Actions simulator guidance (Apple Dev Forums, Orta Therox’s blog) advocating *ephemeral* simulators per job plus unique DerivedData paths.
- Confirmed industry practice: create/boot/delete a fresh simulator in each job, and pass its UDID to xcodebuild (`-destination id=`) to guarantee exclusive ownership.
- Cross-checked prior Issue 12.7 findings—timeouts alone aren’t the culprit; the shared simulator state is.

**Proposed Approach:**
1. Extend `run-xcode-tests.sh` destination resolver to honor env overrides (`ZPOD_SIMULATOR_UDID`, `ZPOD_DERIVED_DATA_PATH`).
2. In CI, generate a unique simulator per matrix job (name scoped by `${{ github.run_id }}-${{ matrix.name }}`), export UDID via `$GITHUB_ENV`, run tests, then delete device in a finally step.
3. Restore matrix parallelism (`max-parallel` > 1) and rely on isolation rather than serialization; keep preflight gating unchanged.
4. Preserve ability to fall back locally—env controls opt-in so developers don’t incur simulator churn.

```mermaid
flowchart TD
    subgraph Job[UI Matrix Job]
      A[Create simulator
         name = run_id + suite] --> B[Export UDID to env]
      B --> C[run-xcode-tests.sh -t suite
              destination id=<UDID>
              derivedData=tmp_ci/<suite>]
      C --> D[Collect artifacts]
      D --> E[Delete simulator]
    end
```

**Open Questions:**
- Do we also need per-job CoreSimulatorService reset (`xcrun simctl shutdown all`) or is deletion sufficient?
- Should IntegrationTests share the same isolation path (likely yes—treat them like UI bundles)?

**Next Steps:**
- Update tooling to respect env-provided simulator / derived-data overrides.
- Rework `.github/workflows/ci.yml` matrix to provision + tear down devices per job and lift `max-parallel`.
- Mirror design in Issue 12.7 log for traceability.

## 2025-10-21 17:45 ET - Tooling + CI Updates Implemented

**Changes Delivered:**
- Extended `scripts/lib/xcode.sh::select_destination` to honor `ZPOD_SIMULATOR_UDID`, validating availability via `simctl list devices` before emitting `-destination id=`.
- Added derived-data plumbing in `run-xcode-tests.sh` so both `test_app_target` and `run_filtered_xcode_tests` respect `ZPOD_DERIVED_DATA_PATH` (mkdir + `-derivedDataPath`).
- Reworked CI matrix job:
  - Lifted `max-parallel` to `3` (no longer fully serialized).
  - Added provisioning step that sanitizes suite name, creates `tmp_ci/DerivedData/<suite>`, picks an iOS 18 runtime, and calls `simctl create` for a per-suite simulator (`zpod-${run_id}-${suite}`).
  - Exported UDID/derived-data path via `$GITHUB_ENV` + step outputs; cleanup step now shuts down & deletes the device and scrubs the derived data directory.
  - Removed the shared "iPhone 16 (CI)" creation path (now redundant).

**Verification:**
- `ZPOD_SIMULATOR_UDID=<existing>` + `ZPOD_DERIVED_DATA_PATH=tmp_ci/DerivedData/local-appsmoke` `./scripts/run-xcode-tests.sh -t AppSmokeTests` ✅
- `ZPOD_SIMULATOR_UDID=<existing>` + `ZPOD_DERIVED_DATA_PATH=tmp_ci/DerivedData/local-ui` `./scripts/run-xcode-tests.sh -t zpodUITests/CoreUINavigationTests` ✅

**Observations:**
- With the UDID override present the harness logs `Using simulator destination from ZPOD_SIMULATOR_UDID`, confirming the new codepath.
- `-derivedDataPath` now appears in every `xcodebuild` invocation, ensuring isolation per job.

**Follow-up:**
- Update AGENTS.md with new env knobs + workflow expectations.
- Monitor first CI run to confirm per-job `simctl delete` keeps the device list clean.
- [2025-10-22 08:20 ET] Addressed GitHub runner failure where `simctl create` returned "Incompatible device" by retrying with a list of known device identifiers (16→13). The provisioning step now attempts each candidate that exists locally and only enables the override once creation succeeds, ensuring older runtimes (iOS 17.*) can fall back to iPhone 15/14 hardware.
- [2025-10-23 01:05 ET] Added `simctl boot` + `bootstatus -b` after successful creation so the simulator is fully initialized before xcodebuild begins; this prevents UI runner startup failures (`AXDisableAccessibilityOnTermination`) observed on cold devices.
- [2025-10-23 09:30 ET] Extended preflight to provision its own simulator/DerivedData bundle (mirrors UI matrix logic) and clean it up afterwards, eliminating AppSmoke hang when xcodebuild waited on an unbooted generic destination for six hours.
- [2025-10-24 07:00 ET] Added a guard in `run-xcode-tests.sh` so UI suites pre-build `zpod.app` inside the job’s DerivedData folder when a custom `ZPOD_DERIVED_DATA_PATH` is in use. Fixes the `ld: file cannot be open()ed` failure (exit 65) seen when the SwipeConfiguration persistence job tried to link against a host app that hadn’t been produced in that sandbox.
...
## 2025-10-26 15:08 ET - Async Stream Bridge Stabilized

- **Root cause**: macOS `swift test` crashed with `Incorrect actor executor assumption` whenever `SettingsManager` subscribed to `UserDefaultsSettingsRepository.settingsChangeStream()`. Capturing the repository via the main-actor manager and calling into the actor from a child `Task` kept the hop on the wrong executor.
- **Fix**: Made `settingsChangeStream()` `nonisolated` and registered continuations asynchronously inside the actor. `SettingsManager` now materializes the stream on the main actor, then consumes it from a detached task while marshalling updates back through `MainActor.run`.
- **Test updates**: Rewrote `Issue05SettingsIntegrationTests` to use the repository harness + `await` flows (no `Task.sleep`). `SettingsManagerFeatureRegistryTests` now waits on `waitForInitialLoad()` instead of polling.
- **Regression**: `swift test` succeeds for `Packages/SettingsDomain` and `Packages/Persistence`; `./scripts/run-xcode-tests.sh -t IntegrationTests` completes green (result bundle `TestResults_20251026_150420_test_IntegrationTests.xcresult`).
- **Follow-up**: Document the new async stream contract in `AGENTS.md`'s persistence section so future agents don’t re-introduce Combine publishers when bridging across actors.

## 2025-10-27 10:05 ET - Preflight Host App Path Fix Plan
- GitHub preflight failed while linking `IntegrationTests` because the build system could not find `zpod.app/zpod` in the provisioned DerivedData bundle.
- `scripts/run-xcode-tests.sh` rebuilds the host app via `build_app_target`, but that helper ignores `ZPOD_DERIVED_DATA_PATH`, so the app lands in default DerivedData and never appears under the CI bundle. The follow-up warning (`Host app still missing...`) confirms the gap.
- Update `build_app_target` so it mirrors the main xcodebuild wrappers: honor `ZPOD_DERIVED_DATA_PATH` and the resolved scheme/destination when building a host target. This keeps host binaries colocated with the test run artifacts.
- Validation strategy: rerun `./scripts/run-xcode-tests.sh -t AppSmokeTests` with `ZPOD_DERIVED_DATA_PATH` exported; confirm the host binary exists and the suite completes without linker errors.

## 2025-10-27 14:12 ET - Persistence Package Regression Debug
- ✅ Preflight now passes, but CI surfaced a new failure in the Persistence package: `EpisodeFilterManagerTests` mutate `@MainActor` state from the default `setUpWithError()` path, which remains nonisolated per XCTest.
- Root cause: the test case is annotated `@MainActor`, so stored properties (repository/service/manager) are main-actor isolated. During setup we instantiate them synchronously, which violates actor rules when Swift enforces isolation for macOS `swift test`.
- Fix approach: drop the class-level `@MainActor` in favor of targeted annotations on the async test methods, and move initialization into `override func setUp() async throws` using `await MainActor.run { EpisodeFilterManager(...) }`. Tear-down will mirror this to release the manager on the main actor.
- Verification plan: run `swift test --package-path Packages/Persistence` followed by `./scripts/run-xcode-tests.sh -t IntegrationTests` to ensure cross-package consumers remain green.

## 2025-10-27 14:38 ET - EpisodeFilterManagerTests Stabilization
- CI still failed: `testCreateSmartList_AddsToSmartLists` intermittently lost the newly appended list because `EpisodeFilterManager`'s async bootstrap overwrote `smartLists` after the test mutation.
- Added deterministic synchronization in `EpisodeFilterManagerTests` by tracking `loadSmartLists` calls on the mock repository and awaiting completion during `setUp()` before exercising mutations.
- Mock repository now records load call counts; tests wait on those counters to avoid racing the initialization task.
- Will rerun `swift test --package-path Packages/Persistence` and the Integration suite locally, then push so CI can re-run the package matrix.

## 2025-10-27 15:02 ET - UITest Host App Linking Fix
- SwipeConfiguration UITest job failed on CI: linker couldn't locate `zpod.app/zpod` under the dedicated DerivedData bundle.
- Root cause: `build_app_target` prebuild step used a generic simulator destination, so the host app compiled into a different simulator runtime path; when the UITest run linked against the job-specific booted UDID, the expected binary wasn't present.
- Patch: when building the host app (`target_label == "zpod"`) and `ZPOD_SIMULATOR_UDID` is provided, force the destination to that UDID so the prebuild mirrors the test run's derived data layout.
- Validation: rerun `./scripts/run-xcode-tests.sh -t zpodUITests/SwipeConfigurationPersistenceUITests` (with and without `ZPOD_DERIVED_DATA_PATH`), then run `./scripts/run-xcode-tests.sh -t IntegrationTests` to confirm no regressions.

## 2025-10-27 15:18 ET - EpisodeFilterManager Load Synchronization
- Persistence package still failed on CI because the EpisodeFilterManager tests sometimes observed zero smart-list loads before assertions, triggering our guard failure.
- Extended the wait helper to interleave an actual `Task.sleep` with yielding, giving the manager's background task time to reach its load call even under CI scheduling.
- Re-ran `swift test --package-path Packages/Persistence` to confirm deterministic behavior.
- UITest + Integration suites already validated earlier; no additional impact from this adjustment.
