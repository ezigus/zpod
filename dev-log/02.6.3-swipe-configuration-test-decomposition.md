...
## 2026-01-12 — Preset Materialization Stability (Local)

**Intent**: Prevent local failures in preset selection tests when the sheet is considered "ready" before presets materialize.

**Hypothesis**: `waitForSectionMaterialization` returns early when the haptics toggle exists, even if the debug materialization probe still reports `Materialized=0`. This allows the test to proceed before presets are materialized.

```mermaid
sequenceDiagram
  participant Test
  participant Sheet
  Test->>Sheet: Open configuration sheet
  Test->>Sheet: Wait for Materialized=1 (probe)
  Test->>Sheet: Locate preset button
```

**Result**: Targeted preset selection test passes (`TestResults/TestResults_20260113_062902_test_zpodUITests-SwipePresetSelectionTests-testDownloadPresetAppliesCorrectly.log`).
