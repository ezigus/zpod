...
- Reduced `max-parallel` from 5 → 3 to avoid resource contention
- Each job provisions dedicated simulator with isolated derived data

**Expected Outcomes**:
- **Reliability**: Eliminates race conditions, each job owns its build
- **Speed**: ~5-7 min total (3 parallel simple + 1 complex vs previous 20-30 min with hangs)
