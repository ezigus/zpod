...
## Next Optimization Actions (runtime without reducing coverage) – Planned

Target: further timing reductions by eliminating redundant waits/queries while keeping full coverage. Execute one action at a time, with targeted swipe-suite runs and PR checklist updates after each action.

1) Sheet readiness gate: Add a helper that opens the sheet, verifies baseline + section materialization once, and returns the container; cache per-test to avoid repeated section waits/materialization scrolls in UIDisplay and preset tests.
2) Preset discovery tightening: Scope preset button lookup to the sheet container (and optional debug ApplyPreset probe); fail fast if absent—no broad scroll sweeps or multiple visibility passes.
3) Navigation de-duplication: Keep the sheet open within a test (UIDisplay/preset) to avoid re-querying nav indicators mid-test; ensure persistence/execution suites relaunch at most once per class.
4) Debug wait cleanup: Remove any remaining no-op `waitForDebugState` polls; rely on single-shot reads + assertions per milestone.
5) Attachment diet: Emit tree/identifier attachments only on failure paths to reduce per-test I/O time.
6) Timeout scale: Pin `UITEST_TIMEOUT_SCALE=1.0` in swipe matrix jobs unless diagnostics show simulator slowness; keep the short defaults established in Phase 1.
7) CI build reuse (optional): Explore reusing preflight host/test artifacts across swipe jobs to cut per-job build time if existing artifact wiring permits.

Validation per action: run the impacted swipe suite(s) (e.g., UIDisplay + PresetSelection after readiness/preset changes), then rerun the full swipe set if changes are cross-cutting.
