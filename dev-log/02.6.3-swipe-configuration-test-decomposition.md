...
- Set `max-parallel: 5` with **future-proof simulator isolation**:
  - **Staggered provisioning**: 0-8s delay per job to avoid simultaneous creation
  - **Active monitoring**: Checks system load, waits if ≥5 simulators active
  - **Retry logic**: Up to 3 attempts for creation/boot with exponential backoff
  - **Resource detection**: Identifies resource exhaustion errors and retries
...
- Each job provisions dedicated simulator with isolated derived data

**Infrastructure Improvements (Future-Proofing)**:

The simulator isolation system now scales beyond 5 parallel jobs:

1. **Deterministic Staggering**: Hash-based delays (0-8s) prevent thundering herd
...
5. **Boot Hardening**: Retries boot process up to 3 times with verification
...
- **Oct 22**: `max-parallel: 3` with simulator isolation ✅ **worked reliably**
...
- **Nov 11**: Enhanced infrastructure + restored `max-parallel: 5` ✅ **future-proof**
