...
Step A — Readiness gate + cache (target suites: UIDisplay, PresetSelection)
- Add a single helper to open the sheet, verify baseline + section materialization once, and cache the sheet container for the test instance.
- Update UIDisplay and preset tests to rely on the cached container; remove any later section/materialization waits in those tests.

Step B — Zero-scroll preset lookup (target suite: PresetSelection)
- In `applyPreset`, look for preset buttons only within the cached container (or debug ApplyPreset probe) and fail fast; drop broad scroll sweeps and redundant visibility loops.

Step C — Post-readiness tightening (target suites: UIDisplay, PresetSelection, ToggleInteraction)
- Remove no-op `waitForDebugState { _ in true }` calls; use a single `expectDebugState` snapshot per milestone.
- After readiness, cap element waits to ~1s; keep a slightly higher cap only for the readiness gate.

Step D — Logging/attachments diet (target: all swipe suites)
- Emit tree/identifier attachments and verbose debug prints only on failure paths to trim I/O on passing runs.

Step E — Navigation/relaunch reuse (target suites: Persistence, Execution)
- Provide a helper to return the cached sheet container when already open; avoid re-opening inside the same test.
- Ensure persistence/execution suites relaunch at most once per class unless the seed changes; assert on unintended relaunches.
