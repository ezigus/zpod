...
CI run #1100 exposed repeated timeouts in `SwipePresetSelectionTests` because the presets section is materialized off-screen inside the sheet, so XCTest never found `SwipeActions.Preset.*` identifiers even after multiple swipe attempts. Fixes:

- Added `UITEST_AUTO_SCROLL_PRESETS` (enabled by `SwipeConfigurationTestSupport`) so the SwiftUI view can reposition the presets section specifically during UI tests. When the flag is present we render presets at the top of the `List` and tag each row with a stable `.id(...)`, ensuring accessibility nodes exist immediately.
- Reworked `ensureVisibleInSheet` to use deterministic coordinate-based scrolling plus scoped identifier logging, and increased the preset-button wait to the longer adaptive timeout so we still catch slow animations.
- Emitted identifier snapshots to both XCTest attachments and stdout (`[SwipeUITestDebug] …`) so future CI captures show exactly which sheet rows are visible when a failure occurs.
