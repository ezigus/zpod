...
- Set `max-parallel: 5` with **future-proof simulator isolation**:
  ...
  - **Active monitoring**: Checks system load, waits if ≥5 simulators active (up to 60s)
  - **Retry logic**: Up to 3 attempts for creation/boot with exponential backoff (3s, 6s, 9s)
  ...
  - **Boot verification**: Confirms simulator responsiveness before proceeding with tests
