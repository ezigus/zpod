...
- Updated `AGENTS.md` workflow guidance to require issue IDs in PR titles and mandate using the PR’s branch name for all commits.

## 2025-11-03 – PlayerFeature macOS Gate (16:55 ET)
- Replaced lingering `Combine` import in `MiniPlayerViewModelTests` with `CombineSupport` and added `Foundation` so `URL`/`TimeInterval` compile on macOS test runners.
- Hardened `EpisodeDetailViewModelTests` by unwrapping optional timestamps and adding an async polling helper to wait for annotation hydration before asserting transcript search results.
- `swift test --package-path` runs for PlayerFeature, DiscoverFeature, LibraryFeature, CoreModels, Networking, Persistence, PlaybackEngine, and SettingsDomain now pass on macOS.
- Full `./scripts/run-xcode-tests.sh` regression completed successfully; new artifacts recorded under `TestResults_20251103_1653xx_*`.

## 2025-11-24 12:15 ET – Lint cleanup + CombineSupport status
- Added targeted SwiftLint fixes (closure formatting, long debug string, preview var rename) and left size/nesting checks enabled; length warnings remain until the planned refactor issue lands.
- Checked history: `Packages/CombineSupport/Tests` has never existed in git (no prior commits). The regression script’s “skipped (no tests)” signal is intentional to flag missing coverage.
- Removed the short-lived `CombineSupportTests` stub so the package continues to report as skipped when no tests are present; this preserves the warning as a health indicator.
