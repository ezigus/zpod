...
## Regression Investigation (2026-02-09 12:27 ET)

**Issue**: Local regression failed with 6 failures in `PlaybackPositionAVPlayerTests` after initial implementation.

**Root Cause**: Our PlaybackStateCoordinator fix to prevent double state emissions removed the `pause()` call from the `.failed` state handler. While this preserved error UI (the original goal), it broke tests that expected the mini player to be in a paused/tappable state.

**Failing Tests**:
- `testMissingAudioURLShowsErrorNoRetry`
- `testNetworkErrorShowsRetryAndRecovers`
- 4 others (all with "Expanded player did not appear" timeout)

**Analysis**: Tests called `expandPlayer()` which taps the mini player. Without `pause()` being called, the playback service stayed in `.failed` state without transitioning to a proper paused state, causing timing issues with mini player interactivity.

**Solution**: Call `pause()` **directly** in the `.failed` case handler (not through `presentAlert`) to ensure:
1. Audio/network activity is stopped
2. Mini player is in tappable/paused state
3. NO double state emission (pause comes from coordinator, not alert-triggered)

**Fix Commit**: `84e0a6f` - Updated PlaybackStateCoordinator.swift and coordinator tests

**Validation**:
- LibraryFeature tests: 21/21 passing
- PlaybackPositionAVPlayerTests: 10/10 passing (2 skipped)
- StreamingInterruptionUITests: 9/9 passing (confirmed no regression in our changes)
