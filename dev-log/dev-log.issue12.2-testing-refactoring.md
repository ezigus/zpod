...
1. **Main Actor Isolation in Setup**: Added `@MainActor` to `setUpWithError()` and `tearDownWithError()` methods in all UI test classes to ensure all UI operations run in the proper actor context
