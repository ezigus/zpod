...
```
┌─────────────────────────────────────────────────────────────┐
│ EpisodePlaybackService Protocol                            │
│ + play(episode:duration:)                                   │
│ + pause()                                                   │
│ + statePublisher: AnyPublisher<EpisodePlaybackState>       │
└─────────────────────────────────────────────────────────────┘
                        ▲
                        │
        ┌───────────────┴───────────────┐
        │                               │
┌───────────────────────┐    ┌──────────────────────┐
│ EnhancedEpisodePlayer │    │ AVPlayerPlaybackEngine│
├───────────────────────┤    ├──────────────────────┤
│ State Management:     │    │ Audio Streaming:     │
│ - Chapters            │◄───│ - AVPlayer instance  │
│ - Speed               │    │ - Time observer      │
│ - Persistence         │    │ - Status KVO         │
│ - Skip silence flag   │    │ - Error handling     │
│ - Volume boost flag   │    │ - Rate control       │
│ - Auto-finish         │    │                      │
│                       │    │ Delegates to:        │
│ Uses:                 │    │ - Audio session      │
│ - Ticker (simulated)  │───►│ - AVPlayerItem       │
│   OR                  │    │ - CMTime observation │
│ - AVPlayerEngine      │    │                      │
│   (real audio)        │    │                      │
└───────────────────────┘    └──────────────────────┘
```
...
1. Play with valid URL → state transitions to playing
2. Position updates emit via callback
3. Pause → audio stops, position frozen
4. Seek → position jumps correctly
5. Play with nil URL → error state
6. AVPlayer status failure → error callback
7. Cleanup on deinit → no crashes
...
- Replace EnhancedEpisodePlayer entirely → Would lose tested logic, high risk
- Fork EnhancedEpisodePlayer → Code duplication, maintenance burden
...
**Alternatives Considered**:
- Protocol-based delegation → More formal but unnecessary ceremony
- Combine publishers → Would work but adds complexity
...
**Alternatives Considered**:
- Configure in AVPlayerPlaybackEngine → Risk of conflicts, duplication
...
- [ ] Starting Episode Playback (lines 55-61)
  - Audio begins streaming through device speakers ✓
  - Position starts at 0 or last saved position ✓
  - Now Playing info updates (via SystemMediaCoordinator) ✓

- [ ] Timeline Advancement During Playback (lines 62-68)
  - Position increases with real time ✓
  - Progress bar advances ✓
  - Lock screen position updates ✓

- [ ] Pausing Playback (lines 69-75)
  - Audio output stops ✓
  - Position remains frozen ✓
  - Progress bar stops ✓

- [ ] Resuming Playback (lines 76-81)
  - Audio resumes from position X ✓
  - Timeline continues advancing ✓

- [ ] Seeking to Position (lines 82-87)
  - Position jumps to Y ✓
  - Audio resumes from Y if playing ✓

- [ ] Background Playback (lines 88-94)
  - Audio continues in background ✓ (via audio session)
  - Lock screen controls functional ✓ (via SystemMediaCoordinator)
  - Position continues advancing ✓

- [ ] Audio Interruption Handling (lines 95-100)
  - Playback pauses on call ✓ (via SystemMediaCoordinator)
  - Resume after call ✓ (via SystemMediaCoordinator)

- [ ] Headphone Disconnect (lines 101-106)
  - Playback pauses immediately ✓ (via SystemMediaCoordinator)
...
**Error Handling**:
- Episode has no audioURL → failPlayback(.episodeUnavailable)
- Network/stream failure → AVPlayer status KVO → onError(.streamFailed)
- EnhancedEpisodePlayer receives error → emits .failed state
