...
## PR Review Follow-up (2026-02-08 11:42 ET)

### Intent

Address incoming PR review comments while preserving Issue 28.1.10 behavior and avoiding scope creep.

### Review Decisions

- **Accepted**: align delete-download view-model guard with effective downloaded state (supports env-seeded `UITEST_DOWNLOADED_EPISODES` rows used by UI tests).
- **Accepted**: only mark episode as `.notDownloaded` after delete call succeeds; retain logging on failure.
- **Accepted**: replace `waitForNonExistence` in `OfflinePlaybackUITests` with existing `waitUntil(.disappeared, timeout:)` helper.
- **Accepted**: deduplicate `normalizeEpisodeIDForView(_:)` by delegating to shared `normalizeEpisodeID(_:)`.
- **Accepted**: add missing `SwipeActionHandlerTests` coverage for `.deleteDownload` callback wiring.
- **Accepted**: force `build test` for `AppSmokeTests` in `run_test_target` when `ZPOD_TEST_WITHOUT_BUILDING=1` to match filtered-path hardening.

### Explicit Non-Change

- **Deferred**: introducing a new user-facing failure banner/alert for delete-download errors. Current scope keeps existing logging-only behavior for failed delete attempts.

## Accessibility Assertion Hardening (2026-02-08 15:47 ET)

### Intent

Add an explicit UI assertion that the swipe delete-download action exposes the expected VoiceOver label text (`Delete Download`) instead of relying only on identifier presence.

### Design

```mermaid
flowchart TD
  A[Open episode list with downloaded episode] --> B[Swipe row left]
  B --> C[Find SwipeAction.deleteDownload button]
  C --> D[Assert button exists]
  D --> E[Assert button label == Delete Download]
  E --> F[Tap and continue existing delete flow]
```

### Planned Change

- Add a label assertion in `OfflinePlaybackUITests.testDeletedDownloadRevertsToStreaming` after the delete swipe action is found and before tapping it.

## CI AppSmoke DerivedData Mismatch Fix (2026-02-08 16:03 ET)

### Symptom

`unit-tests` in GitHub Actions run `21804840863` failed in AppSmoke with:

- `PCH was compiled with module cache path .../HostPrebuild/... but the path is currently .../AppSmoke/...`
- `Missing required module 'SwiftShims'`

This happened after restoring prebuilt host artifacts into a different derived data root and then forcing `build test` for AppSmoke.

### Design

When derived data was restored from a host artifact, keep AppSmoke on `test-without-building` (no rebuild) so module cache and PCH paths stay consistent with the restored prebuild. Continue forcing `build test` for non-restored/local AppSmoke runs.

```mermaid
flowchart TD
  A[AppSmoke run with ZPOD_TEST_WITHOUT_BUILDING=1] --> B{restored marker present?}
  B -- yes --> C[use test-without-building]
  B -- no --> D[force build test]
```

### Planned Changes

- `scripts/ci/restore-host-app.sh`: write a restore marker file in derived data.
- `scripts/lib/harness.sh`: detect marker and bypass AppSmoke build+test forcing for restored-artifact runs.
...
- **2026-02-08 11:42 ET**: Applied PR-review fixes: env-seeded delete-download eligibility in view model, success-only status mutation after delete, UI test wait helper correction, swipe ID normalization dedupe, added `.deleteDownload` handler unit test, and widened AppSmoke filtered routing (`AppSmokeTests*`) in harness. Deferred adding a new user-facing delete failure banner as out-of-scope for this issue.
- **2026-02-08 13:33 ET**: Addressed compile break from dedupe refactor (`Cannot find 'normalizeEpisodeID' in scope` in `EpisodeListView`). Added one file-scoped helper (`normalizedEpisodeIDToken`) and routed both `EpisodeListView.normalizeEpisodeIDForView` and `EpisodeRowView.normalizeEpisodeID` through it.
- **2026-02-08 15:47 ET**: Added explicit VoiceOver label coverage for delete-download swipe action in `OfflinePlaybackUITests` (`SwipeAction.deleteDownload` must expose label `Delete Download` before confirmation tap).
- **2026-02-08 16:03 ET**: Fixed CI AppSmoke derived-data mismatch by marking artifact-restored derived data in `restore-host-app.sh` and teaching harness AppSmoke routing to keep `test-without-building` when that marker is present (avoids HostPrebuild/AppSmoke module cache path conflicts).
- **2026-02-08 16:54 ET**: Marked issue 28.1.10 complete in local/remote issue tracking, prepared merge to `main`, and initiated branch cleanup.
