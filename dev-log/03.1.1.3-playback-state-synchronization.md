...
**Follow-up**:
- Adjusted the integration test setup so the `isLibraryReady` closure is created inside `MainActor.run` to satisfy strict concurrency checks and avoid build failures.
