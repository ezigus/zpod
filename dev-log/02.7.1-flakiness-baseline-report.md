...
CI test flakiness is **critically high**, with a 93.4% CI run failure rate. However, individual test-level failures represent only 0.58% of executions, indicating that a small number of flaky tests are causing widespread CI failures.

### Critical Findings

1. **93.4% of CI runs fail** (85 out of 91 usable runs)
2. **Only 4% of CI runs succeed** (6 out of 150 total runs)
3. **SwipeConfiguration tests account for 100% of analyzed failures**
4. **3 tests are responsible for 50% of all failures**
...
## Top 10 Flakiest Test Suites

| Rank | Suite | Failures | % of Total Failures |
|------|-------|----------|-------------------|
| 1 | **SwipePresetSelectionTests** | 10 | 45.5% |
| 2 | BatchOperationUITests | 4 | 18.2% |
| 3 | SwipeActionManagementTests | 3 | 13.6% |
| 4 | SwipeExecutionTests | 2 | 9.1% |
| 5 | SwipeConfigurationUIDisplayTests | 2 | 9.1% |
| 6 | SwipePersistenceTests | 1 | 4.5% |
| 7-10 | *(No other failing suites found)* | 0 | 0% |

## Top 20 Flakiest Individual Tests

| Rank | Test | Suite | Failures | Severity |
|------|------|-------|----------|----------|
| 1 | **testPlaybackPresetAppliesCorrectly** | SwipePresetSelectionTests | 4 | 🔥 Critical |
| 2 | **testDownloadPresetAppliesCorrectly** | SwipePresetSelectionTests | 4 | 🔥 Critical |
| 3 | **testManagingActionsEndToEnd** | SwipeActionManagementTests | 3 | 🔥 Critical |
| 4 | testOrganizationPresetAppliesCorrectly | SwipePresetSelectionTests | 2 | ⚠️ High |
| 5 | testLeadingAndTrailingSwipesExecute | SwipeExecutionTests | 2 | ⚠️ High |
| 6 | testAllSectionsAppearInSheet | SwipeConfigurationUIDisplayTests | 2 | ⚠️ High |
| 7 | testSeededConfigurationPersistsAcrossControls | SwipePersistenceTests | 1 | ⚡ Medium |
| 8 | testLaunchConfiguredApp_WithForcedOverlayDoesNotWait | BatchOperationUITests | 1 | ⚡ Medium |
| 9 | testCriteriaBasedSelection | BatchOperationUITests | 1 | ⚡ Medium |
| 10 | testBatchOperationCancellation | BatchOperationUITests | 1 | ⚡ Medium |
| 11 | testBasicNavigationToEpisodeList | BatchOperationUITests | 1 | ⚡ Medium |

**Severity Levels**:
- 🔥 **Critical**: >10% failure rate or 3+ failures (requires immediate fix)
- ⚠️ **High**: 5-10% failure rate or 2 failures (high priority)
- ⚡ **Medium**: 2-5% failure rate or 1 failure (monitor)
...
**Tests in Suite**:
- testPlaybackPresetAppliesCorrectly: 4 failures ⚠️
- testDownloadPresetAppliesCorrectly: 4 failures ⚠️
...
**Tests in Suite**:
- testManagingActionsEndToEnd: 3 failures ⚠️
...
1. **Fix Top 3 Critical Tests** (testPlaybackPresetAppliesCorrectly, testDownloadPresetAppliesCorrectly, testManagingActionsEndToEnd)
   - These account for 50% of all failures
   - Fixing these will reduce CI failure rate from 93.4% to ~50%
...
1. ✅ **Phase 1 Complete**: Baseline metrics established
2. ➡️ **Phase 2 Start**: Root cause analysis of top 3 flaky tests (Issue #147)
