...
## 2026-02-07 15:51 EST — Follow-up scope (AC de-overlap + hardening corrections)
- Step #1: tighten Issue `#393` acceptance criteria so `28.1.9` explicitly excludes behavior owned by:
  - `#395` (`28.1.10` swipe delete)
  - `#396` (`28.1.11` network/buffer hook integration)
  - `#269` (PlaybackError UI surface)
- Step #2: fix hardening defects inside `28.1.9` scope:
  - Respect per-episode `UITEST_DOWNLOADED_EPISODES` tokens in row status rendering.
  - Make download status identifiers row-scoped for all rendered statuses.
  - Align offline UI assertions to prefer row-scoped status identifiers.
  - Expose a stable `"Player Interface"` marker in the episode-detail navigation path.

```mermaid
flowchart TD
  A[UITEST_DOWNLOADED_EPISODES tokens] --> B[Normalize per row]
  B --> C{Episode token match?}
  C -->|yes| D[effectiveStatus = downloaded]
  C -->|no| E[effectiveStatus = model status]
  D --> F[Row-scoped status id]
  E --> F
  F --> G[Offline UI tests prefer row status id]
  H[Episode list detail destination] --> I[Expose Player Interface marker]
```

## 2026-02-07 16:08 EST — Phase 2 scope (steps 3-6)
- Goal: complete remaining `28.1.9` hardening tasks after scope de-overlap and identifier fixes.
- Step 3 (stabilize): eliminate flaky streaming setup failures by reusing robust hook lookup + wait helpers.
- Step 4 (skip cleanup): remove dead/unreachable bodies under unconditional `XCTSkip` and keep issue traceability concise.
- Step 5 (documentation): add explicit offline/streaming coverage + blocker mapping to `zpodUITests/TestSummary.md`.
- Step 6 (validation): run targeted suite command and capture pass/skip/fail evidence for AC review.

```mermaid
flowchart TD
  A[Streaming test starts] --> B[Launch with UITEST_* override]
  B --> C[Navigate to Player tab]
  C --> D[Lookup hook control by id+label fallback]
  D -->|not found| E[XCTSkip Issue 28.1.11 #396]
  D -->|found| F[Run assertion using wait helpers]
  F --> G[No fixed sleep; use waitUntil]
```

## 2026-02-07 17:00 EST — Local issue-file parity + closure audit
- Goal: add missing local `Issues/28.1.9-...md` so `28.1.9`, `28.1.10`, and `28.1.11` all have local issue docs.
- Follow-up: audit `#393` acceptance criteria against current branch/PR state and list only remaining closure work.

```mermaid
flowchart TD
  A[Load GitHub issue #393 ACs] --> B[Map ACs to repo evidence]
  B --> C[Separate blocked vs unblocked]
  C --> D[Identify closure-only gaps]
```

## 2026-02-07 17:05 EST — PR review follow-up before merge
- Accepted:
  - Strengthened `testPlaybackAdaptsToPoorNetwork` so it verifies buffering signal + no error alert, not just `playerView.exists`.
  - Converted simulation setup preconditions (navigate to Player tab, `Player Interface` reachable) from `XCTSkip` to assertion failures since these are not `#396`-scoped blockers.
  - Corrected `zpodUITests/TestSummary.md` references to actual repo paths (`zpod/spec/...`) and local issue file (`Issues/28.1.9-ui-test-hardening-offline-streaming-playback.md`).
- Deferred by scope:
  - Kept hook-transition `XCTSkip` behavior tied to `#396` when simulation controls are present but behavioral transitions are still blocked by integration work outside `28.1.9`.
