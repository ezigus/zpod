...
## 2025-12-28 16:45 ET – AppSmoke Crash Triage (MPMediaItemArtwork)

**Intent**: Fix the AppSmoke crash during `SystemMediaCoordinatorTests` caused by actor-isolation violations in artwork handling.

**Findings**:
- Crash report `zpod-2025-12-28-163122.ips` shows `EXC_BREAKPOINT` on `dispatch_assert_queue` from `SystemMediaCoordinator.makeArtwork(from:)`.
- `MPMediaItemArtwork` request handler runs on a MediaPlayer access queue (non-main).
- The handler closure inherits `@MainActor` isolation via `SystemMediaCoordinator`, triggering the runtime trap.

**Plan**:
1. Make artwork construction nonisolated so the request handler is not main-actor isolated.
2. Ensure the closure captures only immutable image data (no `self`).
3. Re-run AppSmoke tests to confirm no crash.
