...
## 2026-01-02 — Seek Reset Instrumentation Plan

### Intent
- Add UI-test gated logging to identify whether playback state restoration or view-model updates are resetting position to 0 after a seek.
- Log only anomalous backward jumps so the output stays readable during ticks.

### Design Sketch
```mermaid
sequenceDiagram
  participant Test as PlaybackPositionUITests
  participant App as App
  participant PSC as PlaybackStateCoordinator
  participant VM as ExpandedPlayerViewModel

  Test->>App: launch (UITEST_POSITION_DEBUG=1)
  App->>PSC: restorePlaybackIfNeeded()
  PSC-->>App: inject paused state (if resume data)
  Test->>App: seek slider to 50%
  App->>VM: playback state updates
  VM-->>Test: log backward jump if position drops to 0
```

### Updates
- Added UI-test gated logging for playback restore injection and backward position jumps.
- Skipped playback resume restoration in UI test mode and cleared resume state on restore attempt.
- Captured the slider value at the first observed advancement to avoid stale post-wait reads.
- Emitted UITest breadcrumbs at info level to verify seek flow steps and slider values.
...
### Current Status - ✅ COMPLETE (2025-12-28)

- ✅ All code changes implemented
- ✅ Unit tests written (19 test cases total)
- ✅ Tests completed and passing
- ✅ Code review remediation complete
- ✅ Integration validation complete (CI passing)
- ✅ Full regression passing
- ✅ Engine layer 100% complete

**UI Integration**: Deferred to Issue 03.3.7 (Player Tab Playback Wiring)
- Mini-player timeline updates → Requires UI wiring
- Expanded player scrubber → Requires UI wiring
- Now Playing lock screen → Deferred to Issue 03.3.5 (MPNowPlayingInfoCenter)

### Completion Summary

**Engine Layer**: ✅ Complete
- Position advancement working (0.5s tick interval)
- Pause/resume functionality verified
- Seeking updates position immediately
- Ticker lifecycle managed correctly
...
**Follow-up Issues**: UI integration work tracked separately
