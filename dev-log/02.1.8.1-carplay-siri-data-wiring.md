...
### 2025-11-02 17:10 ET - Access Control Fix
**Status**: Compilation error resolved

**Issue Found**:
- Build failed during AppSmokeTests with error:
  ```
  ZpodApp.swift:168:54: error: 'resolve' is inaccessible due to 'internal' protection level
  let dependencies = CarPlayDependencyRegistry.resolve()
  ```
- The `resolve()` method in `CarPlayDependencyRegistry` was marked `static` (internal) instead of `public static`
- This prevented `ZpodApp.swift` from accessing the dependency registry from outside the LibraryFeature package

**Fix Applied**:
- Updated `CarPlayDependencies.swift` line 110:
  ```swift
  // Before:
  @MainActor
  ...
  ```
- This aligns with other public methods in the registry (`configure(_:)`, `configure(podcastManager:)`, `reset()`)
- Syntax check passed after fix (276 files)

**Root Cause**:
- Original implementation assumed `resolve()` would only be called within LibraryFeature
- However, `ZpodApp.swift` (in the main app target) needs to call `resolve()` to trigger Siri-initiated playback
- Missing `public` modifier prevented cross-target access

### 2025-11-02 17:15 ET - Test Location Fix
**Status**: Resolved linker errors

**Issue Found**:
- Build failed during AppSmokeTests with undefined symbols:
  ```
  Undefined symbol: SharedUtilities.SiriMediaSearch.TemporalReference
  Undefined symbol: SharedUtilities.SiriMediaLibrary.load(...)
  ```
- `SiriIntentIntegrationTests.swift` was located in `IntegrationTests/` folder (main Xcode project target)
- The IntegrationTests target doesn't have SharedUtilities linked as a framework dependency
- Other integration tests use CoreModels, SearchDomain, TestSupport - none try to import SharedUtilities directly

**Fix Applied**:
- Moved `SiriIntentIntegrationTests.swift` from `IntegrationTests/` to `Packages/SharedUtilities/Tests/SharedUtilitiesTests/`
  ```bash
  mv IntegrationTests/SiriIntentIntegrationTests.swift \
     Packages/SharedUtilities/Tests/SharedUtilitiesTests/SiriIntentIntegrationTests.swift
  ```
- Changed `@testable import SharedUtilities` to `import SharedUtilities` (no longer need internal access)
- Test file now runs as part of SharedUtilities package test suite where it belongs
- Syntax check passed after move (276 files)

**Rationale**:
- Siri integration tests are testing SharedUtilities package APIs (SiriMediaLibrary, SiriMediaResolver, SiriMediaSearch)
- Package tests should live with the package they're testing
- Avoids Xcode project configuration complexity (linking SharedUtilities to IntegrationTests target)
- Matches pattern used by other package tests (CoreModels, SearchDomain, etc.)
