...
- **Notifications configuration** – create a dedicated feature/controller/view so alert and sound toggles live in the registry instead of the legacy `SettingsManager`.
- **Appearance & accessibility toggles** – modularize theme, font-size, and related settings called out in Issue 05.1 using the shared toggle/slider rows.
- **Smart list automation controls** – surface background refresh and filtering options via the registry to replace ad-hoc wiring inside LibraryFeature.
- **Playback preset enhancements** – once the above land, extend playback settings with richer preset descriptions or editing using the shared components.
- **Per-podcast download retention** – eventually expose overrides for individual podcasts, building on the shared picker/stepper rows.
