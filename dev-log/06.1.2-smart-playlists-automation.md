...
## 2026-02-21 — App Layer Wiring

Wired `SmartPlaylistViewModel` into the live Playlists tab (`PlaylistTabView` in LibraryFeature/ContentView.swift). Previously `smartViewModel` was always `nil` so the smart playlist section was hidden. Fixed in commit `d1bfeaf`:

- Added `@State private var smartViewModel: SmartPlaylistViewModel`
- Backed by `InMemorySmartPlaylistManager` with live `allEpisodesProvider` from `podcastManager`
- Playback callbacks (`onPlayAll`, `onShuffle`) wired through CarPlay queue manager
- `PlaylistFeatureView` now receives `smartViewModel` — smart playlist section and enhanced toolbar are shown

Build and AppSmoke (59 tests): 0 failures. Pushed to PR #415.

## 2026-02-21 16:31 EST — Correctness Follow-up (Issue #187)

### Intent

Investigate open PR/issue feedback and resolve remaining correctness gaps in smart-playlist UI behavior:

1. Smart-playlist row counts should stay accurate when the backing episode set changes.
2. Creation-form live preview should refresh when rule logic (`AND`/`OR`) changes, not only when rule rows change.

### Findings

- `SmartPlaylistViewModel` caches row counts during `reload()`, but `reload()` only runs on playlist CRUD events. If episode metadata changes later (new episodes, played/download status updates), cached counts can become stale.
- `SmartPlaylistCreationView` refreshes preview on `rules`, `sortBy`, and `maxEpisodes` changes, but not on `logic` changes. This can show stale preview results after toggling AND/OR.

### Design

```mermaid
flowchart TD
    A[SmartPlaylistSectionView render] --> B[Request cached counts snapshot]
    B --> C[ViewModel computes episode signature]
    C --> D{Signature changed?}
    D -- Yes --> E[Recompute all smart-playlist counts]
    D -- No --> F[Reuse cached counts]
    E --> G[Return counts to rows]
    F --> G
```

Implementation plan:
- Add a lightweight episode-input signature in `SmartPlaylistViewModel` and refresh cached counts when signature changes.
- Read counts once per section render via a snapshot API to avoid per-row re-evaluation churn.
- Add `logic` to `SmartPlaylistCreationView` preview refresh triggers.
- Add/extend unit tests to cover cache refresh behavior when episode inputs mutate.

### Implementation

Applied the planned fixes in PlaylistFeature:

- `SmartPlaylistViewModel`
  - Added `cachedEpisodeSignature` tracking and `cachedEpisodeCountsSnapshot()` API.
  - `cachedEpisodeCount(for:)` now reads from the refreshed snapshot.
  - Added `refreshCachedEpisodeCountsIfNeeded(force:)` and `episodeSignature(for:)` so count cache updates whenever episode inputs change.
  - `reload()` now forces count-cache recomputation after playlist CRUD reloads.
- `SmartPlaylistSectionView`
  - Pulls a single count snapshot per render and reads per-row counts from that snapshot.
- `SmartPlaylistCreationView`
  - Added `.onChange(of: logic)` to keep preview results in sync when user toggles AND/OR.
- Tests
  - Added `testCachedEpisodeCountRefreshesWhenEpisodeProviderChanges()` to lock in the cache invalidation behavior.

### Validation

- `./scripts/run-xcode-tests.sh -t PlaylistFeature` initially failed as expected on the new test (stale count bug reproduced).
- Re-ran `./scripts/run-xcode-tests.sh -t PlaylistFeature` after implementation:
  - **75 tests, 0 failures** (PlaylistFeature package).
