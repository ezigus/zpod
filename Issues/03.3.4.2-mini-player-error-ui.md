...
### ViewModel Changes
- [x] `MiniPlayerDisplayState` struct has new `error: PlaybackError?` field
- [x] `handlePlaybackStateChange(.failed)` passes error to `displayState`
- [x] Non-error states set `error: nil` in `displayState`
- [x] `retryPlayback()` method added to `MiniPlayerViewModel`

### View Changes
- [x] Error overlay appears in mini-player when `displayState.error != nil`
- [x] Error icon (exclamation triangle) is visible
...
- [x] Error overlay has accessibility identifier `MiniPlayer.ErrorOverlay`
- [x] Retry button has accessibility identifier `MiniPlayer.RetryButton`
- [x] VoiceOver reads error state appropriately
- [x] Error clears when new episode starts playing (error becomes nil)
