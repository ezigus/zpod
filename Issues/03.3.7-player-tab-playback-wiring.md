...
The Player tab in `ContentView.swift` currently uses placeholder `@State` values instead of observing the real `EpisodePlaybackService`. Wire the mini-player and expanded player to observe actual playback state.
...
- **Mini-Player Shows Current Playback State** - Title, position, progress bar, play/pause state
- **Mini-Player Controls** - Play/pause toggle functionality
- **Expanded Player Real-Time Updates** - Scrubber updates, elapsed time, seeking
- **Mini-Player Persistence** - Visible across tab navigation
...
### Integration Tests
- Full flow: tap episode → play → mini-player updates
- Seeking → position updates in UI
