...
- Player tab uses production `EnhancedEpisodePlayer` with `AVPlayerPlaybackEngine` (unless `UITEST_DISABLE_AUDIO_ENGINE=1`).
- Feed parsing supplies real `audioURL` values; mini-player/full-player share the same playback service.
- Offline preference is injected via `DownloadCoordinatorBridge.localFileURL(for:)`.
