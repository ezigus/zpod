...
**GitHub Issue**: [#285](https://github.com/ezigus/zpod/issues/285)

## Description
Shift the mini-player from an always-visible bar to a hidden-by-default experience with a persistent pill handle. The pill handle should always be available (regardless of playback) and expand on swipe-up or tap to reveal the mini-player controls. The mini-player can be collapsed at any time without stopping playback.
...
- **And** The pill handle does not obstruct tab bar taps or primary content
- **And** The pill handle has an accessibility label (e.g., "Show mini-player") and identifier

### Scenario 2: Expand Mini-Player
- **Given** The pill handle is visible
- **When** I swipe up on the pill handle or tap it
...
- Default to the collapsed pill handle on app launch and when playback state changes.
- Keep the pill handle visible even when playback is idle.
...
- Use minimal safe-area inset for the pill handle; expanded state should sit above the tab bar without blocking it.
