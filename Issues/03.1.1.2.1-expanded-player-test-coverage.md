...
**Scope Update (2026-01)**: The expanded player will also be embedded in the Player tab. Tests should cover both sheet-based presentation (from mini-player) and in-tab presentation (see `03.1.1.7-player-tab-full-player`).
...
**Enhancement**: Add `testExpandedPlayerDismissesOnSwipeDown` (sheet) and `testPlayerTabShowsExpandedPlayer` (tab) to PlayerPlaybackInteractionTests

**Rationale**: Validate that swipe-down gesture properly returns to the mini-player and that the Player tab embeds the expanded player without dismissal
...
func testExpandedPlayerDismissesOnSwipeDown() throws {
  ...
  // Verify mini-player returns (collapsed pill or expanded mini-player)
  let pillHandle = miniPlayerPillElement()
  XCTAssertTrue(pillHandle.waitForExistence(timeout: 2))
...
func testPlayerTabShowsExpandedPlayer() throws {
  launchApp()
  ...
  XCTAssertTrue(playerTab.waitForExistence(timeout: adaptiveShortTimeout))
  playerTab.tap()
