...
- **Unit Tests** (`zpodTests`): Pure unit tests for app-level logic only
  - No UI test code
...
- Running `./scripts/run-xcode-tests.sh` with no arguments executes:
  ...
  4. All SwiftPM package tests (`-b all` packages)
  5. App unit tests (`-t zpod` → zpodTests only)
...
1. Audit current test targets:
   - List all test classes in `zpodTests/`
...
3. Move tests to correct targets:
   ...
   - Pure unit tests → `zpodTests/` or package tests
...
2. Each CI job runs specific target only:
   - `unit`: `./scripts/run-xcode-tests.sh -t zpodTests`
