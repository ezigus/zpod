...
**Estimated Effort**: 30 minutes (actual: 1 hour with discovery)

## Description

After completing Issues 03.3.4.1-03.3.4.4, return to Issue 03.3.2.7 to implement the error-related edge-case tests. **Discovery**: Test infrastructure already existed in ContentView - just needed to implement test bodies.

## Problem Statement - RESOLVED

**Initial State** (2026-01-08):
- Issue 03.3.2.7 had 10 edge-case tests
- 7 tests passing ✅
- 2 error tests were stubs awaiting implementation
- Investigation revealed infrastructure already complete

**Final State** (2026-01-09):
- Discovered `UITEST_AUDIO_OVERRIDE_MODE` and `UITEST_AUDIO_OVERRIDE_URL` already implemented
- Implemented both error test bodies
- Tests verify error UI, messages, and retry button behavior
- Brings 03.3.2.7 to 90% (9/10 tests passing)

## Implementation

### Test 1: Missing Audio URL
```swift
func testMissingAudioURLShowsErrorNoRetry()
- Uses UITEST_AUDIO_OVERRIDE_MODE="missing" to create episode with nil audioURL
- Verifies error view appears
- Checks for "doesn't have audio available" message
- Confirms NO retry button (not recoverable)
```

### Test 2: Network Error
```swift
func testNetworkErrorShowsRetryAndRecovers()
- Uses UITEST_AUDIO_OVERRIDE_URL="http://invalid-host..." to trigger network error
- Verifies error view appears (with network timeout)
- Checks for network-related error message
- Confirms retry button EXISTS (recoverable)
```

## Infrastructure Already Complete

ContentView.swift already had:
- `UITEST_AUDIO_OVERRIDE_MODE` support (lines 344-349)
- `UITEST_AUDIO_OVERRIDE_URL` support (lines 351-368)
- Proper nil handling and fallback logic
