...
**Impact**:
- ❌ Users lose all podcasts when app closes
- ❌ Test code shipping in production builds
- ❌ No real persistence layer for podcast library
- ❌ Violates architectural guidelines (Persistence layer missing)

## Acceptance Criteria

- [x] ~~Create `PodcastRepository.swift` in `Packages/Persistence/Sources/Persistence/`~~ ✅ Created `SwiftDataPodcastManager.swift` in `zpod/Persistence/` (see deviation notes)
- [x] Implement `PodcastManaging` protocol with SwiftData persistence ✅
- [x] Support all operations from protocol: ✅
  - [x] `all()` - Fetch all podcasts ✅
  - [x] `find(id:)` - Find by ID ✅
  - [x] `add(_:)` - Add podcast ✅
  - [x] `update(_:)` - Update podcast ✅
  - [x] `remove(id:)` - Remove podcast ✅
  - [x] `findByFolder(folderId:)` - Filter by folder ✅
  - [x] `findByFolderRecursive(folderId:folderManager:)` - Recursive folder filter ✅
  - [x] `findByTag(tagId:)` - Filter by tag ✅
  - [x] `findUnorganized()` - Find unorganized podcasts ✅
- [x] Implement SwiftData model for `Podcast` entity ✅ (PodcastEntity.swift)
- [x] Preserve Siri snapshot persistence functionality ✅
- [x] ~~Add migration support for future schema changes~~ ⏳ Deferred (not needed yet)
- [x] Unit tests with in-memory ModelContainer for all operations ✅ (27 tests)
- [x] Integration tests with persistent ModelContainer ✅ (8 tests)
- [x] Thread-safe implementation with proper actor isolation ✅ (serial DispatchQueue)
...
- [x] PodcastRepository implementation complete with all protocol methods ✅
- [x] SwiftData models defined and tested ✅
- [x] Unit tests passing (>90% code coverage) ✅
- [x] Integration tests passing ✅
- [x] Documentation written with code examples ✅
- [x] Dev-log entry created documenting design decisions ✅
...
**Test Coverage**:
- 27 unit tests (`SwiftDataPodcastManagerTests.swift`)
