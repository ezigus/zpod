...
The `EnhancedEpisodePlayer` emits a `.playing` state once when play is called, but never advances `currentPosition`. The `Ticker` protocol and `TimerTicker` implementation exist in `PlaybackTypes.swift` but are never instantiated or used.
...
```swift
// PlaybackTypes.swift - EXISTS but never used
