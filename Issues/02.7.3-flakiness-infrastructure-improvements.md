...
## ⚠️ Updated After Phase 1 Correction
...
From Phase 2 (#02.7.2):
- ✅ Build/infrastructure failure root causes categorized (simulator, build, infra, etc.)
- ✅ Test execution failure root causes categorized (timing, race conditions, etc.)
- ✅ Infrastructure gaps identified for both levels
...
## 🔴 CRITICAL: Build/Infrastructure Resilience (Addresses 75% of failures)

### 1. CI Workflow Simulator Retry Logic ⭐ **CRITICAL Priority**
  ...
  run: |
    ...
    for attempt in {1..3}; do
      ...
      if [ $attempt -lt 3 ]; then
        echo "⚠️ Boot failed, retrying in 10 seconds..."
        sleep 10
      fi
    done
...
### 2. SPM Dependency Caching ⭐ **CRITICAL Priority**
...
- name: Resolve SPM Dependencies with Retry
  run: |
    for attempt in {1..3}; do
      ...
      if [ $attempt -lt 3 ]; then
        echo "⚠️ Resolution failed, clearing cache and retrying..."
...
### 3. Workflow Timeout Adjustments ⭐ **HIGH Priority**
...
## ⚠️ HIGH: Test Infrastructure (Addresses 25% of failures)
...
## CI Failure Analysis (Dec 7, 2025)

### Latest CI Run Analysis

**Run**: #19996247579 (Dec 7, 2025)
**Branch**: `infra/phase3-test-helpers` (PR #155)

#### Failed Jobs Summary

| Job | Tests Failed | Root Cause Category | Fix Location |
|-----|--------------|---------------------|--------------|
| SwipePresetSelectionTests | 3/3 | Test logic/timing | PR #152, #154 |
| SwipeExecutionTests | All | CI infrastructure | CI workflow |
| ContentDiscoveryUITests | 2 | CI infrastructure | CI workflow |

#### Detailed Failure Analysis

**1. SwipePresetSelectionTests (Test Logic Issues)**
- `testDownloadPreset`: "Lost connection to application" - App crashed
- `testOrganizationPreset`: Preset button didn't appear (341s timeout)
- `testPlaybackPreset`: Preset button didn't appear (231s timeout)

**Root Cause**: SwiftUI lazy rendering + CI timing race condition. Preset buttons aren't materialized before test attempts to tap them.

**Fix**: PR #152 adds 500ms stability wait after section materialization. PR #154 ensures SwipeActionConfigurationView forces section materialization.

**2. SwipeExecutionTests (CI Infrastructure)**
- Error: "Process spawn via launchd failed because device is not booted"
- Simulator wasn't booted before tests ran (on-demand boot failed)

**3. ContentDiscoveryUITests (CI Infrastructure)**
- Error: "Failed to get background assertion for target app: Timed out"
- Simulator resource contention prevented background assertion acquisition

### PR Dependency Analysis

| PR | Purpose | Fixes SwipePreset? | Status |
|----|---------|-------------------|--------|
| #155 (current) | Infrastructure helpers | ❌ Helpers exist but not integrated | Open |
| #152 | Stability wait + test fixes | ✅ Yes | Open |
| #154 | Lazy unmaterialization pattern | ✅ Yes | Open |

### Recommended Resolution Path

**Option A (Recommended): Sequential Merge**
1. Merge PR #154 (pattern documentation + SwipeActionConfigurationView fix)
2. Merge PR #152 (stability wait + test support improvements)
3. Then merge PR #155 (infrastructure helpers - now useful for future)

**Option B: Consolidate into PR #155**
- Cherry-pick critical fixes from PR #152 and #154 into #155
- Single comprehensive PR with all fixes

### CI Infrastructure Improvements Needed

1. **Explicit Simulator Boot Verification**: Before running tests, verify simulator is fully booted (not just created)
2. **Increased Warmup Time**: Add delay between simulator boot and test execution
3. **Reduce Parallelism**: Consider reducing from 5 to 4 parallel UI test jobs to reduce resource contention

---

## Future Enhancements

The following improvements were identified during code review but are not blocking for initial infrastructure release. These should be evaluated and either implemented or split into separate issues:

### Timeout Configuration
- **Description**: Extract timeout constants to a centralized config
- **Rationale**: Currently timeouts are hardcoded (waitBriefly: 0.5s, waitForPageLoad: 2.0s, etc.). Centralizing would make them easier to adjust for CI vs local testing.
- **Effort**: Low
- **Priority**: Nice-to-have
- **Status**: TODO - Evaluate if needed

### Infrastructure Metrics Collection
- **Description**: Add metrics collection for infrastructure usage
- **Details**: Track how often helpers are used, average wait times, success rates
- **Benefits**: Data-driven optimization, identify which helpers are most valuable
- **Effort**: Medium
- **Priority**: Nice-to-have
- **Status**: TODO - Consider for observability improvements

### iOS Version Compatibility
- **Description**: Add @available annotations if iOS version compatibility matters
- **Context**: Current infrastructure doesn't use version-specific APIs, but future additions might
- **Effort**: Low (add annotations as needed)
- **Priority**: Low
- **Status**: TODO - Address when/if compatibility issues arise

### Scroll Velocity Logging
- **Description**: Add diagnostic logging for scroll velocity and distance
- **Details**: Track scroll events to debug scrolling-related test issues, measure scroll performance
- **Benefits**: Better debugging of SwiftUI lazy-loading issues, understand scroll behavior patterns
- **Effort**: Low
- **Priority**: Nice-to-have
- **Status**: TODO - Consider for debugging improvements

### Unit Tests for Infrastructure Helpers
- **Description**: Add unit tests for infrastructure helpers using mock XCUIElement
- **Rationale**: Currently helpers are tested implicitly through UI test usage. Explicit unit tests would provide faster feedback and better coverage
- **Effort**: Medium (requires XCUIElement mocking strategy)
- **Priority**: Nice-to-have
- **Status**: TODO - Evaluate testing strategy for infrastructure code
