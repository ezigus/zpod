...
- Currently emits `.episodeUnavailable` → needs change to `.missingAudioURL`
- `onError` callback already wired from `AVPlayerPlaybackEngine` → `EnhancedEpisodePlayer.failPlayback()`
...
**Important**: The `EnhancedEpisodePlayer` state machine already feeds `MiniPlayerViewModel`/`ExpandedPlayerViewModel`. Ensure the new errors flow through so the published `.failed` state includes the mapped `PlaybackError`. This ensures the UI overlays trigger correctly and retry actions land back on the same playback service.
