...
- [ ] User taps Play → Audio streams through device speakers
...
The following specifications cover this epic:

- `zpod/spec/spec.md` - Podcast Playback Control and Playback Error Handling scenarios
- `zpod/spec/spec.md` - User Interface and Experience (mini-player access and full player)
...
│                        UI Layer                              │
│  ┌─────────────┐  ┌─────────────────┐  ┌─────────────────┐  │
│  │ MiniPlayer  │  │ ExpandedPlayer  │  │ SystemMedia     │  │
│  │ ViewModel   │  │ ViewModel       │  │ Coordinator     │  │
│  └──────┬──────┘  └────────┬────────┘  └────────┬────────┘  │
│         │                  │                     │           │
│         └──────────────────┼─────────────────────┘           │
│                            │                                 │
│                   statePublisher (Combine)                   │
└────────────────────────────┼────────────────────────────────┘
                             │
┌────────────────────────────┼────────────────────────────────┐
│                   Playback Engine                            │
│                            │                                 │
│  ┌─────────────────────────▼─────────────────────────────┐  │
│  │           AVPlayerPlaybackEngine (NEW)                 │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌──────────────┐   │  │
│  │  │  AVPlayer   │  │ TimeObserver│  │ Ticker       │   │  │
│  │  │  (audio)    │  │ (position)  │  │ (fallback)   │   │  │
│  │  └─────────────┘  └─────────────┘  └──────────────┘   │  │
│  └───────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
                             │
...
```
Phase 1: Foundation
├── 03.3.1 Position Ticking Engine (unblocks UI timeline)
└── 03.3.3 Feed Parsing (provides real audio URLs)

Phase 2: Core Playback
├── 03.3.2 AVPlayer-Backed Engine (actual audio)
└── 03.3.7 Player Tab Wiring (connect UI to service)

Phase 3: Polish
├── 03.3.5 Now Playing Sync (lock screen accuracy)
├── 03.3.4 Unplayable UX (error handling)
└── 03.3.6 DI Wiring (production safety)
```
