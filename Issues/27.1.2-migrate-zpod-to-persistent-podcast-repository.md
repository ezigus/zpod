...
1. **Siri snapshot verification**: add an automated check that persistent storage updates flow into shared app-group snapshots.
2. **Spec alignment**: persistence-after-restart scenarios added to `spec/discovery.md` and `spec/customization.md`; keep them in sync with tests.
...
- [ ] Siri snapshot generation → verify written to app group
