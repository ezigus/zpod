...
- [#396](https://github.com/ezigus/zpod/issues/396) / Issue `28.1.11`: network/buffer simulation hook integration
...
- [ ] **Streaming suite hardening:** all `StreamingInterruptionUITests` scenarios that are not blocked by `#396` or `#269` are active and pass locally and in CI; blocked scenarios skip gracefully with explicit issue-traceable `XCTSkip` reasons.
...
4. Ensure blocked scenarios skip with issue references (`#395`, `#396`, `#269`) instead of hard failures.
...
- Blocked feature dependencies tracked externally: `#395`, `#396`, `#269`
