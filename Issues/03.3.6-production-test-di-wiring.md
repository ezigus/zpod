...
- `CarPlayDependencies.defaultDependencies` wires `AVPlayerPlaybackEngine` when `UITEST_DISABLE_AUDIO_ENGINE != "1"` and falls back to ticker for UI tests.
- `EnhancedEpisodePlayer` accepts `localFileProvider` and is fed by `DownloadCoordinatorBridge.shared` for offline preference.
- `ZpodApp` and LibraryFeature production flows use the real engine; test mode remains configurable via env flags.
...
### Build Verification
- Release build → uses production service
- Debug build without flag → uses production service
- Debug build with `--use-mock-playback` → uses mock service
