...
## Acceptance Criteria
- [x] `update()` syncs episodes using upsert logic
- [x] New episodes from feed are inserted
- [x] Existing episodes updated (metadata only, preserve user playback state)
- [x] Episodes with `playbackPosition > 0` never deleted even if removed from feed
- [x] Downloaded episodes (`downloadStatus == .downloaded`) preserved
- [x] Favorited/bookmarked episodes preserved
- [x] Tests for sync edge cases (new episode, updated metadata, removed from feed but has state)
...
- **Current gap to fix**: `SwiftDataPodcastRepository.update(_:)` (Packages/Persistence/Sources/Persistence/SwiftDataPodcastRepository.swift:94-116) never deletes episodes that are absent from the incoming payload; stale rows linger and continue to surface in CarPlay/search. The sync work here must delete missing episodes that lack user state (playback/download/favorite/bookmark/played) and add tests covering: removal without user state deletes, removal with user state keeps, mixed state scenarios.
...
**Test Cases:**
- [x] `testSyncDeletesStaleEpisodesWithoutUserState` - Episodes without user state removed
- [x] `testSyncKeepsEpisodeWithPlaybackPosition` - Playback position > 0 preserved
- [x] `testSyncKeepsPlayedEpisode` - Played episodes preserved
- [x] `testSyncKeepsDownloadedEpisode` - Downloaded episodes not deleted
- [x] `testSyncKeepsFavoritedEpisode` - Favorited episodes not deleted
- [x] `testSyncKeepsBookmarkedEpisode` - Bookmarked episodes not deleted
- [x] `testSyncKeepsArchivedEpisode` - Archived episodes preserved
- [x] `testSyncKeepsRatedEpisode` - Rated episodes preserved
- [x] `testSyncMixedScenarioDeletesOnlyStale` - Mixed state scenario (includes archived/rated) works correctly
