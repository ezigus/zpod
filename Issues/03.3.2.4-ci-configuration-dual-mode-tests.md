...
Add CI workflow configuration for the two new playback test suites (Ticker and AVPlayer). Both suites run as blocking jobs in parallel with existing UI test jobs.

## Problem Statement

**Current State**:
- Playback position tests not integrated into CI
- No automated validation of AVPlayer integration in pull requests
- Risk of merging code that breaks playback without detection

**Target State**:
- `UITests-PlaybackTicker` job runs ticker tests (~3 minutes)
- `UITests-PlaybackAVPlayer` job runs AVPlayer tests (~5 minutes)
- Both jobs are BLOCKING (must pass to merge)
- Jobs run in parallel with existing UI tests (Wave 2 execution)
...
Preflight (builds host app, ~8 min)
         │
         ├─────> unit-tests + package-tests (parallel)
         │
         └─────> ui-tests matrix (max-parallel: 5)
                 
                 Wave 1 (5 jobs run in parallel):
                 ┌─────────────┬─────────────┬─────────────┬─────────────┬─────────────┐
                 │ Navigation  │ ContentDisc │ Playback    │ CorePlayback│ BatchOps    │
                 │ (~8 min)    │ (~6 min)    │ (~10 min)   │ (~8 min)    │ (~12 min)   │
                 └─────────────┴─────────────┴─────────────┴─────────────┴─────────────┘
                 
                 Wave 2 (2 jobs run in parallel after Wave 1):
                 ┌─────────────┬─────────────┐
                 │ PlaybackTick│ PlaybackAV  │
                 │ (~3 min)    │ (~5 min)    │
                 └─────────────┴─────────────┘
```

**Total CI time impact**: +5 minutes (Wave 2 runs after Wave 1 completes)

## Acceptance Criteria

- [ ] `UITests-PlaybackTicker` job added to ci.yml matrix
- [ ] `UITests-PlaybackAVPlayer` job added to ci.yml matrix
- [ ] Both jobs provision dedicated simulators (same pattern as existing jobs)
- [ ] Both jobs use same timeout as other UI jobs (15 minutes)
- [ ] Both jobs are blocking (no `continue-on-error: true`)
- [ ] Workflow dispatch supports `ui-playback` option for targeted testing
- [ ] CI pipeline completes in <35 minutes total
...
|------|----------------|---------|
| `.github/workflows/ci.yml` | +10 | Add matrix entries |

## Implementation Steps

### Step 1: Add Matrix Entries (10 minutes)

1. Open `.github/workflows/ci.yml`
2. Find the `ui-tests` job matrix (around line 801-812)
3. After the last entry (`UITests-BatchOperations`), add:
...
3. Update the `ui-tests` job condition (around line 790):
...
|--------|--------|-------|-------|
| UI matrix jobs | 5 | 7 | +2 |
| Max parallel | 5 | 5 | 0 |
| Wave 1 duration | ~12 min | ~12 min | 0 |
| Wave 2 duration | N/A | ~5 min | +5 min |
...
|-----------|--------------|--------|
| Ticker job in matrix | Check ci.yml line ~815 | ⏳ |
| AVPlayer job in matrix | Check ci.yml line ~817 | ⏳ |
| Workflow dispatch works | Manual trigger succeeds | ⏳ |
| Both jobs pass in CI | Check Actions logs | ⏳ |
| No flakiness | 3/3 passes for both jobs | ⏳ |
| CI time acceptable | <35 minutes total | ⏳ |
| Jobs are blocking | No continue-on-error | ⏳ |
