...
- [ ] UI test validates **Episode Missing Audio URL** error state (no retry) per `zpod/spec/playback.md`.
- [ ] UI test validates **Network Error During Playback** shows retry, and **Successful Retry After Error** clears the error.
- [ ] Integration/UI test simulates **Audio Interruption Handling** (pause on interruption, resume after interruption ends).
- [ ] UI test validates **Playing an Episode with Custom Speed** updates position advancement rate (blocked until speed controls are exposed in UI).
- [ ] Manual test checklist covers **Headphone Disconnect** behavior if route-change simulation is not feasible.
