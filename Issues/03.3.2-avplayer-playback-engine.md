...
public final class AVPlayerPlaybackEngine: EpisodePlaybackService, EpisodeTransportControlling {
    ...
    private let stateSubject = CurrentValueSubject<EpisodePlaybackState, Never>(.idle)
    ...
    public func play(episode: Episode, duration: TimeInterval) {
        guard let audioURL = episode.audioURL else {
            stateSubject.send(.failed(episode, 0, duration, .missingAudioURL))
            return
        }
...
- [ ] Audio plays through device speakers when play() called
- [ ] Audio plays through Bluetooth/AirPods when connected
- [ ] Background playback continues when app backgrounded
- [ ] Play/pause controls work correctly
- [ ] Seek to position works (scrubbing)
- [ ] Skip forward/backward work
- [ ] Position updates emit every 0.5 seconds via statePublisher
- [ ] Audio session category is `.playback` with `.spokenAudio` mode
- [ ] Interruption handling (pause on call, resume after)
- [ ] Route change handling (pause when headphones unplugged)
- [ ] Error state emitted when URL invalid or network fails
- [ ] Clean up on stop/deinit (remove observers, release player)
...
| File | Action |
|------|--------|
| `Packages/PlaybackEngine/Sources/PlaybackEngine/AVPlayerPlaybackEngine.swift` | CREATE |
| `Packages/PlaybackEngine/Sources/PlaybackEngine/PlaybackError.swift` | CREATE (error types) |
| `Packages/PlaybackEngine/Sources/PlaybackEngine/EpisodePlaybackService.swift` | May need protocol updates |
| `Packages/PlaybackEngine/Tests/PlaybackEngineTests/AVPlayerPlaybackEngineTests.swift` | CREATE |
...
**Medium** - 4-6 hours
- AVPlayer API is straightforward
- Most complexity is in error handling and edge cases
- Testing requires careful mock setup
