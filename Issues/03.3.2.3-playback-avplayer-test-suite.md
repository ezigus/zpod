...
- [ ] `PlaybackPositionAVPlayerTests.swift` created with 5 test methods
- [ ] All tests launch WITHOUT `UITEST_DISABLE_AUDIO_ENGINE` flag
- [ ] All tests conform to `PlaybackPositionTestSupport` protocol
- [ ] Test 1: Position advances during AVPlayer playback
- [ ] Test 2: Position stops when AVPlayer paused
- [ ] Test 3: Position resumes after AVPlayer resumed
- [ ] Test 4: Seeking calls AVPlayer.seek(to:) and updates position
- [ ] Test 5: Mini-player reflects AVPlayer state changes
- [ ] All tests use 10s timeout for position advancement
- [ ] All tests use ±0.5s tolerance for position stability
- [ ] All tests complete in <2 minutes total
- [ ] All tests pass locally via `./scripts/run-xcode-tests.sh -t zpodUITests/PlaybackPositionAVPlayerTests`
