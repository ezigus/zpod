...
- [x] `testNetworkErrorMessageDisplays()` is un-skipped and passes:
...
- [x] `testRetryButtonAvailableAfterError()` is un-skipped and passes:
...
- [x] A simulation hook for error injection is added (e.g., `TestHook.SimulatePlaybackError` button gated by environment flag)
- [x] The hook follows the existing notification-bridge pattern from 28.1.11 (button → notification → manager → engine)
- [x] Zero runtime cost when simulation environment flags are absent
- [x] `EpisodeDetailView` simulation controls are split by flag:
