...
- `testAddingSingleLeadingAction()`
  - Open sheet → Add "Play" to leading → Verify action appears
  - **Extract from**: `addAction()` helper usage in manual configuration
  
- `testAddingSingleTrailingAction()`
  - Open sheet → Add "Download" to trailing → Verify action appears
  - **Extract from**: `addAction()` helper usage in manual configuration
  
- `testRemovingLeadingAction()`
  - Open sheet → Remove "Mark Played" → Verify action removed
...
- `testPlaybackPresetAppliesCorrectly()`
  - Open sheet → Select "Playback" preset → Verify leading=["play", "addToPlaylist"], trailing=["download", "favorite"]
  - **Extract from**: `testPresetSelectionUpdatesSummaryForEachPreset()` preset loop
  
- `testOrganizationPresetAppliesCorrectly()`
  - Open sheet → Select "Organization" preset → Verify leading=["markPlayed", "favorite"], trailing=["archive", "delete"]
  - **Extract from**: `testPresetSelectionUpdatesSummaryForEachPreset()` preset loop
  
- `testDownloadPresetAppliesCorrectly()`
  - Open sheet → Select "Download" preset → Verify leading=["download", "markPlayed"], trailing=["archive", "delete"]
  - **Extract from**: `testPresetSelectionUpdatesSummaryForEachPreset()` preset loop
...
- `testHapticToggleEnablesDisables()`
  - Open sheet → Toggle haptics off → Verify toggle state + debug summary
  - **Extract from**: `setHaptics()` / `assertHapticsEnabled()` logic
  
- `testHapticStylePickerChangesValue()`
  - Open sheet → Enable haptics → Select "Rigid" → Verify style selected
  - **Extract from**: Haptic style picker interaction in `setHaptics()`
  
- `testFullSwipeToggleLeadingTrailing()`
  - Open sheet → Toggle full swipe for leading/trailing → Verify state
...
- `testLeadingSwipeActionsExecute()`
  - Seed config with leading actions → Swipe episode right → Verify action appears
  - **Extract from**: First half of `testSeededSwipeActionsExecuteInEpisodeList()`
  
- `testTrailingSwipeActionsExecute()`
  - Seed config with trailing actions → Swipe episode left → Verify action appears
...
- [ ] Original `SwipeConfigurationUITests.swift` is removed and replaced with the six focused suites described above, each owning ≤ 5 tests scoped to a single responsibility.
- [ ] All original swipe behaviors (display, presets, toggles, persistence, execution) remain covered by the new suites; every scenario executes in ≤ 5 minutes and performs at most one app relaunch per test.
- [ ] Shared helpers are factored into modular files, each ≤ 400 lines, with only the minimal APIs exposed publicly (navigation, seeding, swipe revelation, toggles).
- [ ] CI matrix runs the six suites in parallel with deterministic job names, and `TestSummary.md` documents the mapping between jobs and functionality.
- [ ] Instrumentation (debug state logging, identifier snapshots) remains available for troubleshooting.
- [ ] Before/after metrics for suite runtime are captured in the dev log and linked from the issue.
