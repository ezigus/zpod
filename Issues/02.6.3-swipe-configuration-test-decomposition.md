...
- `testConfigurationSheetDisplaysAllDefaultElements()`
  - **Single test consolidates**: Opens sheet → Verifies all sections materialize → Validates default actions → Checks haptic controls
  - **Rationale**: These 3 checks share the same setup (open sheet), so consolidating avoids 2× unnecessary app launches
...
- `testManagingActionsEndToEnd()`
  - **Single test consolidates**: Add actions to cap → Verify limit enforcement → Remove action → Add trailing action → Verify save enabled
...
- `testActionLimitEnforcementLeading()`
  - Open sheet → Add actions until cap reached → Verify "Add" button disappears
...
- **Option A (Current - 3 separate tests)**: Each preset gets its own test for granular CI failure reporting
  ...
  - **Total**: 330s (with 3× app launch overhead = 54s wasted)
  
- **Option B (Consolidated - 1 test recommended)**: Loop through all 3 presets in single test
  - `testAllPresetsApplyCorrectly()` - ~240s (3 presets × 80s each)
  - **Saves**: 90s (2× app launches eliminated)
...
- `testSeededConfigurationPersistsAcrossControls()`
  - **Single test consolidates**: Seed all settings (actions + haptics + full-swipe) → Launch app → Verify all persisted
  - **Rationale**: Persistence is a single concern - "do settings survive relaunch?" Testing each setting separately adds 2× extra relaunches (36s overhead).
...
- `testLeadingAndTrailingSwipesExecute()`
  - **Single test consolidates**: Seed both leading+trailing → Launch → Execute leading swipe → Execute trailing swipe → Verify both
  - **Rationale**: Execution is a single workflow - "do seeded actions work in episode list?" Splitting adds 1× extra relaunch (18s overhead).
...
## Current Implementation Snapshot (2025-11-20)

- ✅ **Test layout**: All 12 swipe tests now live across six focused files (`SwipeConfigurationUIDisplayTests` → `SwipeExecutionTests`). UIDisplay/preset/toggle suites each have three short tests, while action/persistence/execution remain consolidated workflows per the overhead analysis.
- ✅ **Helper hygiene**: After splitting the monolithic test support into targeted extensions (`+SheetUtilities`, `+ToggleDiagnostics`, etc.), every helper file sits below the 400-line ceiling (see `dev-log/02.6.3…` table). Shared helpers cache the sheet container (`reuseOrOpenConfigurationSheet`) and gate relaunches (`launchSeededApp(resetDefaults:)`).
- ✅ **Timeout policy**: `postReadinessTimeout` distinguishes the initial readiness gate from follow-up assertions (~1s local / 1.5s CI). Persistence/execution keep their longer adaptive waits only where relaunches/seed streams really require it.
- ✅ **Preset materialization**: `SwipeActionConfigurationView` now forces a scroll to the sentinel `swipe-presets-bottom` anchor (gated by `UITEST_SWIPE_PRELOAD_SECTIONS`) so preset rows exist immediately; debug shortcuts rely on the same post-readiness timeout as the rest of the suite instead of extending waits.
- ✅ **Logging discipline**: Success-path tree dumps were removed; diagnostics now emit only on failure. The run script (`scripts/run-xcode-tests.sh`) records per-phase timing and appends the timing block to each `TestResults_*.log` artifact for traceability.
- ✅ **Latest targeted runtimes**: UIDisplay/Preset/Toggle suites finish in ~6–7s test time, ActionManagement in ~4s, and the seeded Persistence/Execution suites complete in ~63–69s (single relaunch each). Result paths are documented in `dev-log/02.6.3…` (2025-11-19 entries).
- 🔄 **Outstanding acceptance items**: document the final layout in `TestSummary.md`/Issue acceptance, capture a green CI run that shows the six swipe jobs executing in parallel (≤6 minutes), and write the close-out summary with helper sizes + wait reductions.

## Acceptance Criteria (updated)

1. ✅ Split `SwipeConfigurationUITests.swift` into six focused suites with a total of 12 tests (3+3+3+1+1+1 layout).
2. ✅ Ensure every helper extension stays ≤400 lines and document the line counts.
3. ✅ Enforce readiness caching and single-launch guards so each suite launches at most once per class (persistence/execution allowed one seed relaunch).
4. ✅ Cap post-readiness waits at ~1s (scaled in CI) and move success-path logging to failure-only branches.
5. 🔄 Update `TestSummary.md`, this Issue, and `dev-log/02.6.3…` with the final suite responsibilities, helper counts, and per-suite runtimes.
6. 🔄 Produce a CI evidence run showing the six swipe jobs running in parallel with ≤6-minute total duration (or document the measured time if higher) and link the workflow in the dev-log/Issue.
...
- [x] All original swipe behaviors covered; every test executes ≤5 minutes with ≤1 relaunch
- [x] Shared helpers factored into modular extensions (8 files: base + 7 extensions), each ≤400 lines
...
**Action Management** (4 tests → 1 consolidated): ✅ **ALL COVERED**

- Original: Add leading | Add trailing | Remove action | Limit enforcement
- Current: `testManagingActionsEndToEnd` covers:
  - Add 2 leading actions to cap ✅
  - Verify limit (add button disappears) ✅
  - Remove leading action ✅
  - Add trailing action ✅
  - Verify save enabled ✅
- **Benefit**: Workflow tested in realistic context, better failure isolation

**Persistence** (3 tests → 1 consolidated): ✅ **MORE COVERAGE**
...
- Current: `testSeededConfigurationPersistsAcrossControls` validates:
  - Leading actions persist ✅
  - Trailing actions persist ✅
  - Haptic toggle persists ✅
  - Haptic style persists ✅
  - Full-swipe leading persists ✅
  - Full-swipe trailing persists ✅
- **Benefit**: All settings validated together; comprehensive UserDefaults encoding test

**Swipe Execution** (2 tests → 1 consolidated): ✅ **ALL COVERED**

- Original: Leading swipe executes | Trailing swipe executes
- Current: `testLeadingAndTrailingSwipesExecute` validates:
  - Leading swipe reveals actions ✅
  - Leading action executes ✅
  - Trailing swipe reveals actions ✅
  - Trailing action executes ✅
...
- **Total scenarios covered**: 18 planned scenarios ✅
...
1. **Refine Persistence Tests** – Split the persistence scenarios into smaller tests that reuse a single launch (e.g., configure → assert → mutate → assert) so each test performs at most one relaunch.
2. **Stub Expensive UI Flows** – Replace playlist-selection and action-cap workflows with lightweight stubs or mocked sheets to avoid multi‑second waits every run.
3. **Add Timing Guardrails** – Instrument each suite to fail fast if a test exceeds 5 minutes, and add dev-log guidance for keeping helper extensions under 400 lines when new scenarios are added.
