...
### SwiftLens Alignment
As files are split, migrate SwiftUI interactions to the SwiftLens SwiftUI testing library (https://github.com/gahntpo/SwiftLens) where feasible. New or refactored test classes should prefer SwiftLensTestSupport for view state and interaction, leaving XCUITest-only coverage for flows that cannot be driven by SwiftLens. This refers to the SwiftUI testing library, not the SwiftLens MCP server.
...
- [ ] Prefer SwiftLensTestSupport in new or refactored SwiftUI tests; document any remaining XCUITest-only flows
...
#### 1. PlaybackUITests.swift (18 tests) → 4 files
...
#### 2. CoreUINavigationTests.swift (14 tests) → 4 files
...
#### 3. ContentDiscoveryUITests.swift (12 tests) → 3 files
