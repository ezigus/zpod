...
- [ ] `PlaybackPositionTestSupport.swift` created with protocol and helpers
- [ ] `startPlayback()` navigates to library and starts episode
- [ ] `expandPlayer()` expands mini-player to full player
- [ ] `getSliderValue()` reads progress slider value
- [ ] `extractCurrentPosition(from:)` parses time string to seconds
- [ ] `waitForPositionAdvancement(beyond:timeout:)` waits for position increase
- [ ] `waitForUIStabilization(afterSeekingFrom:...)` waits for seek completion
- [ ] `verifyPositionStable(at:forDuration:tolerance:)` verifies no drift
- [ ] `PlaybackTestMode` enum added to `UITestHelpers.swift`
- [ ] `configuredForUITests(playbackMode:)` factory method added
- [ ] `CarPlayDependencies.swift` respects `UITEST_DISABLE_AUDIO_ENGINE=0`
- [ ] All helpers use adaptive timeouts from `SmartUITesting`
- [ ] Debug logging respects `UITEST_POSITION_DEBUG` environment variable
