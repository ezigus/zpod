...
public enum RetentionPolicy: Codable, Equatable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        case "keepLatest":
            ...
            self = .keepLatest(max(1, count))
        case "deleteAfterDays":
            let days = try container.decode(Int.self, forKey: .value)
            self = .deleteAfterDays(max(ValidationConstants.minRetentionDays, min(ValidationConstants.maxRetentionDays, days)))
        case "deleteAfterPlayed":
...
public struct DownloadSettings: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        self.maxConcurrentDownloads = max(ValidationConstants.minConcurrentDownloads, min(ValidationConstants.maxConcurrentDownloads, maxConcurrentDownloads))
...
public struct NotificationSettings: Codable, Equatable, Sendable {
    ...
    public let customSounds: [String: String]
    public init(
...
public struct PodcastPlaybackSettings: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        self.podcastId = podcastId
        self.playbackSpeed = playbackSpeed.map { max(ValidationConstants.minPlaybackSpeed, min(ValidationConstants.maxPlaybackSpeed, $0)) }
        self.skipForwardInterval = skipForwardInterval.map { max(ValidationConstants.minSkipInterval, min(ValidationConstants.maxSkipInterval, $0)) }
        self.skipBackwardInterval = skipBackwardInterval.map { max(ValidationConstants.minSkipInterval, min(ValidationConstants.maxSkipInterval, $0)) }
        self.introSkipDuration = introSkipDuration.map { max(0, $0) }
        self.outroSkipDuration = outroSkipDuration.map { max(0, $0) }
    }
}
