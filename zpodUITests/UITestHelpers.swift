...
extension UITestFoundation {
  ...
  var postReadinessTimeout: TimeInterval {
    // Increased from 1.5s→2.5s→3.0s (local) and 2.0s→3.0s→4.0s (CI) to handle resource
    // exhaustion in full regression runs and fresh app launches after termination.
    // Tests still proceed immediately when elements appear quickly - this is a maximum
    // ceiling, not a fixed delay.
    let baseTimeout = ProcessInfo.processInfo.environment["CI"] != nil ? 4.0 : 3.0
    return baseTimeout * timeoutScale
  }
