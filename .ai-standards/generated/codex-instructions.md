...
- Primary UI development target is iPhone unless task scope explicitly targets CarPlay/watchOS UI flows.
- Default eligible profiles: `ios-swift`, `ui-testing`, `carplay`.
- Shipwright profile is conditional and not default.
...
- Prefer explicit waits over brittle timing assumptions.
- Keep test setup isolated and reproducible.
- Do not use fixed sleeps for polling; use waits/expectations first.
- Fixed sleeps are last resort only and should be justified in test context.
- Prefer `.matching(identifier:).firstMatch` locator strategy for duplicate-ID-safe queries.
- Always scroll to the target immediately before interacting when SwiftUI lazy list unmaterialization is possible.

- IF: Task involves Swift, Xcode, iOS, or watchOS code/build/test concerns
...
- Respect actor isolation and async/await patterns.
- Prefer deterministic tests and explicit simulator configuration.
- Do not change actor isolation on XCTest overrides.
- Validate API signatures/enum cases/isolation before coding against existing interfaces.
- Prefer `Sendable` across actor boundaries and `@MainActor` for UI-facing APIs.

- IF: Task involves CarPlay features, templates, HIG compliance, or driving-safe interactions
...
- Prioritize driving-safe interaction patterns.
- Maintain accessibility and template constraints.
- Maintain minimum 44pt touch targets.
- Keep list depths under 100 items.
- Include cancel actions in alerts.
